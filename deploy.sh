...
if [[ -n "$1" ]] ; then
   if [ "$arg1" = "test" ]; then
        ...
        sudo ssh -tt ec2-user@test " cd "$DIR" && docker system prune -a --volumes -f && bash tests.sh && exit "
        echo finish tests successfully
   elif [ "$arg1" = "prod" ]; then
       echo product;
       sudo scp docker-compose.yml .env ec2-user@prod:/home/ec2-user/Project_prod
       sudo ssh -tt ec2-user@prod " cd "$DIR2" && docker system prune -a --volumes -f && docker pull urielwo/final_project:latest && docker-compose up -d && exit "
   else
